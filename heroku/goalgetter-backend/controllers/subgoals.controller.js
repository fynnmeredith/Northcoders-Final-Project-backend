...
var postSubgoal = function (req, res, next) {
    ...
    if (!objective || !end_date || !owner) {
        ...
        return checkExists_1.checkUserExists(owner)
            ...
            return subgoals_model_1.insertSubgoal(goal_id, objective, start_date, end_date, owner, target_value, unit);
        })
...
var getSubgoalBySubgoalId = function (req, res, next) {
    ...
    if (!Number.isInteger(parseInt(subgoal_id))) {
        ...
        return subgoals_model_1.selectSubgoalBySubgoalId(subgoal_id)
            .then(function (subgoal) {
            res.status(200).send({ subgoal: subgoal });
        })["catch"](next);
    }
...
var patchSubgoalStatus = function (req, res, next) {
    ...
    if (!Number.isInteger(parseInt(subgoal_id))) {
        ...
        return checkExists_1.checkSubgoalExists(subgoal_id)
            ...
            return subgoals_model_1.updateSubgoalStatus(subgoal_id, status, date);
        })
            .then(function (subgoal) {
            res.status(200).send({ subgoal: subgoal });
        })["catch"](next);
...
var patchSubgoalProgress = function (req, res, next) {
    ...
    if (!date || !value) {
        ...
        return subgoals_model_1.selectSubgoalBySubgoalId(subgoal_id)
            ...
            return subgoals_model_1.updateSubgoalProgress(subgoal_id, date, value, subgoal.progress, subgoal.target_value);
        })
            .then(function (subgoal) {
            res.status(200).send({ subgoal: subgoal });
        })["catch"](next);
...
var getSubgoalsByGoalId = function (req, res, next) {
    ...
    if (!Number.isInteger(parseInt(goal_id))) {
        ...
        return checkExists_1.checkGoalExists(goal_id)
            ...
            return subgoals_model_1.selectSubgoalsByGoalId(goal_id);
        })
    ...
    if (from_date && !/\d{4}-\d{2}-\d{2}/.test(from_date)) {
        ...
        if (formattedToDate_1 &&
            ...
            return checkExists_1.checkUserExists(username)
                ...
                return subgoals_model_1.selectSubgoalsByUser(username, formattedFromDate_1, formattedToDate_1);
            })
                .then(function (subgoals) {
                res.status(200).send({ subgoals: subgoals });
            })["catch"](next);
